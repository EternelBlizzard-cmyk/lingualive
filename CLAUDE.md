...
13. **`backdrop-filter` casse `position: fixed`** : un élément avec `backdrop-filter` devient le repère de positionnement de ses descendants `fixed` — la barre d'onglets flottante du bas se calait sur la `.topbar` et remontait tout en haut de l'écran. Même effet avec `filter`, `transform` et `will-change`. Si un élément fixé atterrit au mauvais endroit, chercher un de ces quatre chez un ancêtre. Une **couche masquée plein écran** (`mask-image` sur un élément couvrant tout l'écran) au-dessus du mode appel étouffe par ailleurs le compositeur : capture d'écran impossible sur PC alors que la page répond, et saccades probables sur téléphone.
